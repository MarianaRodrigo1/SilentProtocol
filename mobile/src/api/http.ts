import { HTTP_DEFAULT_REQUEST_TIMEOUT_MS } from '../constants';
import { API_BASE_URL } from './config';

export type ApiErrorCode = 'NETWORK_ERROR' | 'TIMEOUT' | 'HTTP_ERROR';
...
async function request(
  ...
  const { signal, dispose } = abortSignalWithTimeout(HTTP_DEFAULT_REQUEST_TIMEOUT_MS, userAbort);
...
export async function probeBackendReachable(timeoutMs: number): Promise<boolean> {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}${withQuery('/agents', { limit: 1 })}`, {
      method: 'GET',
...
export const apiMessages = {
  ...
  loadAgents: 'Could not load agents list.',
